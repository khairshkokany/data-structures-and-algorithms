...
function transformToLis(obj){
  // Solution code here...
  let array = Object.entries(obj).map((item) => {
...
let findMaleAndFemale = (items) => {
  // Solution code here...
  let regex = /\d/g;
  return items.filter((arr) => {
      return !regex.test(arr.name) && arr.name;
    })
    .map((arr) => {
      return arr.name;
    })
...
let findShortest = (items) => {
  // Solution code here...
  let red = items[0].name;
  items
    .map((obj) => {
      return obj.name;
    })
